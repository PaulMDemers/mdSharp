...
| --- | --- | --- | --- |
| Streets of Rage title | `streets-title.ogg` or `streets-title.flac` | VGMPF `The Street of Rage` | Direct per-track game-rip OGG, listed as track 01, 1:38. Recommended source for the Streets title reference. |
| Streets of Rage title/stage tracks | `streets-title.vgm` / rendered WAV | OC ReMix Streets of Rage VGM archive | Exact VGM register-log archive with `The Street of Rage` and 15 other tracks. Useful for YM/PSG register-level synthesis checks. |
| Streets of Rage official CD | `streets-title.flac` | Bare Knuckle Original Soundtrack | Official digitally remastered album, track `The Street of Rage`, 1:41. Appropriate when sourced from a legally owned CD or digital purchase. |
...
For chip-level checks, download the OC ReMix VGM ZIPs and render individual `.vgm` files through `--vgm-render`; these files are better suited to validating YM2612/PSG synthesis than full in-game Z80 timing.

## Reference Emulator Capture

For end-to-end game audio, a known-good emulator capture is usually cleaner than YouTube or album audio because it preserves boot timing, Z80 driver behavior, sound effects, and the exact game mix. MAME is the preferred local capture source because it can write the final mixer output directly to WAV and stop after a fixed emulated duration.
