...
public sealed class Sh2Cpu
{
    ...
    public bool TryFastForwardShllRotclDtBfsAddLoop(int maxCycles, out int cycles)
    {
        ...
        const int MaxBurstIterations = 1024;
        cycles = 0;
        if (maxCycles < CyclesPerIteration ||
            Halted ||
            HasAcceptablePendingInterrupt ||
        ...
        uint loopPc = PC;
        if (!peekBus.TryPeekWord(loopPc, out ushort shiftOpcode) ||
            !peekBus.TryPeekWord(loopPc + 2, out ushort rotateOpcode) ||
            !peekBus.TryPeekWord(loopPc + 4, out ushort dtOpcode) ||
            !peekBus.TryPeekWord(loopPc + 6, out ushort branchOpcode) ||
            !peekBus.TryPeekWord(loopPc + 8, out ushort addOpcode))
        {
            return false;
        }

        if ((shiftOpcode & 0xF0FF) != 0x4000 ||
        ...
        uint iterations = Math.Min(count, (uint)Math.Min(maxCycles / CyclesPerIteration, MaxBurstIterations));
        if (iterations == 0)
        {
            return false;
        }
        ...
        {
            uint shiftValue = R[shiftRegister];
            uint carry = shiftValue >> 31;
            R[shiftRegister] = shiftValue << 1;

            uint rotateValue = R[rotateRegister];
            R[rotateRegister] = (rotateValue << 1) | carry;

            R[countRegister]--;
            R[addRegister]--;
        }

        bool completed = R[countRegister] == 0;
        SetT(completed);
        PC = completed ? loopPc + 12 : loopPc;
        cycles = checked((int)(iterations * CyclesPerIteration));
        Cycles += cycles;
