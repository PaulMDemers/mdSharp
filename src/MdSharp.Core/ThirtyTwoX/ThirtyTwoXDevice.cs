...
public sealed class ThirtyTwoXDevice
{
    ...
    private bool _sdkWordStreamTerminatorPendingYieldRelease;
    ...
    private int StepSh2Cpu(int cpuIndex, int cycleBudget)
    {
        ...
        if (canProbeFastPath && IsSh2FastPathGroupEnabled("early"))
        {
            ...
            if (IsSh2FastPathGroupEnabled("memstore") &&
                (((nextOpcode & 0xF00F) == 0x2001) ||
                    ...
                    ((nextOpcode & 0xFF00) == 0x8F00) ||
                    ((nextOpcode & 0xF000) == 0x7000)) &&
                cpu.TryFastForwardMovWStoreDtBfsAddImmediateLoop(
                    Math.Min(cycleBudget, 4 * Sh2FrameBufferWordFillLoopMaxBurstIterations),
                    _sh2WordWriters[cpuIndex],
                    out fastCycles))
            {
                RecordSh2FastPath(fastCycles);
            ...
            if (IsSh2FastPathGroupEnabled("memscan") &&
                nextOpcode == 0x60C0 &&
                cpu.TryFastForwardDoomByteHighBitPatchDtBfLoop(
                    Math.Min(cycleBudget, 7 * 4096),
                    _sh2ByteReaders[cpuIndex],
                    _sh2WordReaders[cpuIndex],
                    _sh2WordWriters[cpuIndex],
            ...
            if (IsSh2FastPathGroupEnabled("memtransform") &&
                (nextOpcode == 0x7AFE || nextOpcode == 0x79FE) &&
                cpu.TryFastForwardMaskedWordDeltaBackfillLoop(
                    Math.Min(cycleBudget, 130 * 512),
                    _sh2WordReaders[cpuIndex],
                    _sh2WordWriters[cpuIndex],
                    130,
                    out fastCycles))
        ...
        if (canProbeFastPath && IsSh2FastPathGroupEnabled("poll"))
        {
            ...
            if ((nextOpcode & 0xFF00) == 0xEC00 &&
                cpu.TryFastForwardMovImmediateDtBfRtsDelayLoop(cycleBudget, out fastCycles))
            {
                RecordSh2FastPath(fastCycles);
                AdvanceSh2InternalTimers(cpuIndex, fastCycles);
                return fastCycles;
            ...
            if (((nextOpcode & 0xFC00) == 0xC400 ||
                    ...
                    (nextOpcode & 0xFF00) == 0x8B00) &&
                cpu.TryFastForwardPaddedGbrCmpEqBfPollLoop(cycleBudget, out fastCycles))
            {
                RecordSh2FastPath(fastCycles);
                AdvanceSh2InternalTimers(cpuIndex, fastCycles);
                return fastCycles;
            ...
            if (((nextOpcode & 0xFC00) == 0xC400 ||
                    ...
                    (nextOpcode & 0xFF00) == 0x8B00) &&
                cpu.TryFastForwardPaddedGbrTstBfPollLoop(cycleBudget, out fastCycles))
            {
                RecordSh2FastPath(fastCycles);
                AdvanceSh2InternalTimers(cpuIndex, fastCycles);
                return fastCycles;
            ...
            if ((((nextOpcode & 0xF00F) == 0x6002) ||
                    ((nextOpcode & 0xF000) == 0x5000) ||
                    (nextOpcode == 0x0009) ||
                    ((nextOpcode & 0xF00F) == 0x3000) ||
                    (nextOpcode & 0xFF00) == 0x8800 ||
                    (nextOpcode & 0xFF00) == 0x8B00) &&
                (cpu.TryFastForwardLongRegisterCmpEqBfPollLoop(cycleBudget, out fastCycles) ||
                    cpu.TryFastForwardLongCmpEqBfPollLoop(cycleBudget, out fastCycles) ||
                    cpu.TryFastForwardPaddedLongCmpEqBfPollLoop(cycleBudget, out fastCycles)))
            {
                RecordSh2FastPath(fastCycles);
                AdvanceSh2InternalTimers(cpuIndex, fastCycles);
                return fastCycles;
            ...
            if ((((nextOpcode & 0xF00F) == 0x6001) ||
                    ...
                    nextOpcode == 0x0009 ||
                    (nextOpcode & 0xF000) == 0xE000) &&
                cpu.TryFastForwardWordTstBtsPollLoop(cycleBudget, out fastCycles))
            {
                RecordSh2FastPath(fastCycles);
                AdvanceSh2InternalTimers(cpuIndex, fastCycles);
                return fastCycles;
            ...
            if (((nextOpcode & 0xF00F) == 0x6002 ||
                    ...
                    (nextOpcode & 0xF00F) == 0x2008 ||
                    (nextOpcode & 0xFF00) == 0x8900) &&
                (cpu.TryFastForwardLongTstBtPollLoop(cycleBudget, out fastCycles) ||
            ...
            if ((((nextOpcode & 0xFF00) == 0x8400) ||
                    ((nextOpcode & 0xF00F) == 0x2008) ||
                    ((nextOpcode & 0xFF00) == 0x8900)) &&
                cpu.TryFastForwardByteDisplacementTstRegisterBtPollLoop(cycleBudget, out fastCycles))
            {
                RecordSh2FastPath(fastCycles);
                AdvanceSh2InternalTimers(cpuIndex, fastCycles);
                return fastCycles;
    ...
    private bool TryFastForwardSdkWordStreamHandshakeLoop(Sh2Cpu cpu, int cpuIndex, int cycleBudget, out int cycles)
    {
            ...
            (!activeHostCommandStream || firstWord == 0xFFFF))
        {
            ...
            while (skippedWords < maxSkip)
            {
                ...
                if (!TryPeekSh2Word(nextSource, cpuIndex, out ushort nextWord) || nextWord != firstWord)
                {
                    break;
                }
    ...
    private static bool IsSkippableSdkWordStreamFiller(ushort value)
    {
        return value is 0x0000 or 0xFFFF;
    }

    private void TryReleaseFastForwardedSdkSh2YieldSignal(int cpuIndex, bool keepCompletedStreamDrainActive = false)
    {
        ushort comm = ThirtyTwoXHardwareProfile.CommunicationPortOffset;
        if (ReadBigEndianWord(_systemRegisters, comm + 0x0A) == 0 ||
            !ShouldReleaseSdkHostYieldSignal())
        {
            return;
        }
    ...
    private bool IsSdkWordStreamCommandActive()
    {
        ushort comm = ThirtyTwoXHardwareProfile.CommunicationPortOffset;
        return ReadBigEndianWord(_systemRegisters, comm + 0x0E) == 0x0015 &&
            IsSdkCommandDispatchTablePresent();
    }
    ...
    private bool ShouldReleaseSdkHostYieldSignal()
    {
        return IsSdkHostCommunicationCommandActive() || IsCompletedSdkWordStreamTerminatorPending();
    }

    private bool IsCompletedSdkWordStreamTerminatorPending()
    {
        return _sdkWordStreamTerminatorPendingYieldRelease &&
            IsSdkCommandDispatchTablePresent();
    }
    ...
    public void WriteVdpRegisterByte(ushort offset, byte value, string source = "VDP")
    {
        ...
        if (IsM68kVdpRegisterAccessDenied(source, (ushort)(index & ~1)))
        {
    ...
    public void RestoreState(ThirtyTwoXState state)
    {
        ...
        _sdkWordStreamTerminatorPendingYieldRelease = state.SdkWordStreamTerminatorPendingYieldRelease;
    ...
    {
        ushort comm = ThirtyTwoXHardwareProfile.CommunicationPortOffset;
        bool releaseHostYield = ShouldReleaseSdkHostYieldSignal();
        if (offset != comm + 0x0A ||
            value == 0 ||
            (!releaseHostYield &&
                (ReadBigEndianWord(_systemRegisters, comm + 0x0C) == 0 ||
        ...
        WriteBigEndianWord(_systemRegisters, comm + 0x0A, 0);
        _sdkWordStreamTerminatorPendingYieldRelease = false;
        bool keepHostCommandPending = IsSdkHostCommunicationCommandActive();
        if (!keepHostCommandPending)
        {
    ...
    private void TryReleaseSdkSh2YieldSignalAfterWorkerStep(ushort offset, ushort value, int cpuIndex)
    {
        ...
        if (offset != comm + 0x0A ||
            value == 0 ||
            !IsSdkHostCommunicationCommandActive() ||
    ...
    private void TryCompleteSdkHostCommunicationCommandOnSh2StatusWord(ushort offset, ushort value, int cpuIndex)
    {
        ...
        if (ReadBigEndianWord(_systemRegisters, comm + 0x0E) != 0)
        {
            _sdkWordStreamTerminatorPendingYieldRelease = true;
        }
    ...
    private void ClearCommandInterruptForCpu(int cpuIndex)
    {
        ...
        TryRetireDirectHostCommunicationCommandAfterInterruptClear(cpuIndex);
    }

    private void TryRetireDirectHostCommunicationCommandAfterInterruptClear(int cpuIndex)
    {
        ushort comm = ThirtyTwoXHardwareProfile.CommunicationPortOffset;
        ushort command = ReadBigEndianWord(_systemRegisters, comm + 0x0C);
        ushort selector = ReadBigEndianWord(_systemRegisters, comm + 0x0E);
        if (!IsDirectHostCommunicationCommand(command, selector))
        {
            return;
        }
    ...
    private static bool IsDirectHostCommunicationCommand(ushort command, ushort selector)
    {
        return command != 0 &&
            command <= 0x000F &&
            selector == command;
    ...
    private void TryRequestCommandInterruptFromHostCommunicationCommand(ushort alignedOffset, ushort value)
    {
        ...
        LatchCommandInterruptTargets(targets);
        RequestPendingInterrupts();
    }

    private void LatchCommandInterruptTargets(byte targets)
    {
    ...
    public sealed record ThirtyTwoXState(
        ...
        bool SdkWordStreamTerminatorPendingYieldRelease,
